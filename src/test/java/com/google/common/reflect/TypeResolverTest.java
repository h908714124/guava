...
public class TypeResolverTest extends TestCase {
    ...
    public <T1, T2 extends List<T1>> void testWhere_recursiveMapping() {
        Type t1 = new TypeCapture<T1>() {
        }.capture();
        Type t2 = new TypeCapture<T2>() {
        }.capture();
    ...
    public <T> void testWhere_primitiveArrayMapping() {
        ...
        assertEquals(
                int.class,
                new TypeResolver().where(new TypeCapture<T[]>() {
        ...
        assertEquals(
                ...
                new TypeResolver()
                        .where(
                                ...
                                new TypeCapture<List<? extends String>>() {
                                }.capture())
                        .resolveType(t));
        assertEquals(
                ...
                new TypeResolver()
                        .where(
                                ...
                                new TypeCapture<List<? super String>>() {
                                }.capture())
                        .resolveType(t));
    }
    ...
    public <T> void testWhere_wildcardTypeMapping() {
        ...
        assertEquals(
                ...
                new TypeResolver()
                        .where(
                                new TypeCapture<List<? extends T>>() {
                                }.capture(),
                                new TypeCapture<List<? extends String>>() {
                                }.capture())
                        .resolveType(t));
    ...
    public <T> void testWhere_incompatibleParameterizedTypeMapping() {
        try {
            new TypeResolver().where(new TypeCapture<Iterable<T>>() {
            }.capture(), List.class);
            fail();
        } catch (IllegalArgumentException expected) {
        }
        ...
        try {
            new TypeResolver()
                    .where(
                            ...
                            new TypeCapture<List<? extends Integer>>() {
                            }.capture());
            fail();
        } catch (IllegalArgumentException expected) {
        }
        ...
        try {
            new TypeResolver()
                    .where(
                            ...
                            new TypeCapture<List<? super Integer>>() {
                            }.capture());
            fail();
        } catch (IllegalArgumentException expected) {
        }
    ...
    public <T> void testWhere_mapFromConcreteParameterizedType() {
        try {
            new TypeResolver().where(new TypeCapture<List<String>>() {
    ...
    public <K, V> void testWhere_actualArgHasWildcard() {
        TypeResolver resolver =
                new TypeResolver()
                        .where(
                                new TypeCapture<Iterable<Map<?, V>>>() {
                                }.capture(),
                                new TypeCapture<Iterable<Map<String, Integer>>>() {
                                }.capture());
        assertEquals(
                new TypeCapture<K>() {
                }.capture(), resolver.resolveType(new TypeCapture<K>() {
                }.capture()));
        assertEquals(Integer.class, resolver.resolveType(new TypeCapture<V>() {
        }.capture()));
    }
    ...
    public <T> void testWhere_mapFromWildcard() {
        ...
        assertEquals(
                new TypeCapture<TypedKeyMap<String>>() {
                }.capture(),
                new TypeResolver()
                        .where(
                                new TypeCapture<Map<Integer, T>>() {
                                }.capture(),
                                new TypeCapture<Map<?, String>>() {
                                }.capture())
                        .resolveType(subtype));
    }
    ...
    public <T> void testWhere_mapFromWildcardToParameterized() {
        ...
        assertEquals(
                new TypeCapture<TypedListKeyMap<String>>() {
                }.capture(),
                new TypeResolver()
                        .where(
                                new TypeCapture<Map<List<Integer>, T>>() {
                                }.capture(),
                                new TypeCapture<Map<?, String>>() {
                                }.capture())
                        .resolveType(subtype));
        ...
        Type unused =
                new TypeResolver()
                        .where(
                                ...
                                new TypeCapture<Map<? extends Number, ? extends Number>>() {
                                }.capture())
                        .resolveType(subtype);
    }
    ...
    private static <T> Type aTypeVariable() {
        return new TypeCapture<T>() {
        }.capture();
    }

    private static <T> Type aWildcardType() {
        ParameterizedType parameterizedType =
                (ParameterizedType) new TypeCapture<List<? extends T>>() {
                }.capture();
